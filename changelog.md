...
Model accuracy overhaul: out-of-fold evaluation, calibrated risk
...
- Week-cutoff models (wk 2–10) ship in the model bundle; the live scorer picks
  ...
  median, so the live demo cohort is no longer over-flagged as ~all High.
...
- Risk-band cutoffs are now derived from data at each retrain instead of the
  ...
  The Low line requires a historical failure rate of at most half the
  cohort's base rate.
- All models are strictly behavioral: continuous-assessment marks were
  ...
  Behavioral-only early warning, now reported for weeks 2–10:
